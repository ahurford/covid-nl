...
data = data.frame(dates2 = tail(dates,-1), tests.roll = c(rep(NA,13),values.roll$tests), detected.roll = c(rep(NA,13),values.roll$cases), all = c(rep(NA,7),undetected+active, rep(NA,6)), cases.raw = cases.raw, test.raw = test.raw, positivity.roll = c(rep(NA,13),values.roll$positivity), positivity.raw = cases.raw/test.raw)
return(data)
}
...
data = cbind(data, detected.roll.hub = detected.roll.hub, all.hub = all.hub, positivity.roll.hub = positivity.roll.hub, cases.raw.hub=cases.raw.hub, test.raw.hub = test.raw.hub)
...
gNL2=ggplot() +
  geom_line(data=data, aes(x=as.Date(dates2), y=data$detected.roll.hub), col = cols[5], lty=2)+
  geom_point(data=data, aes(x=as.Date(dates2), y=data$cases.raw.hub), col = cols[5], pch=2)+
  geom_point(data=data, aes(x=as.Date(dates2), y=data$cases.raw), col = cols[5])+
  geom_line(data=data, aes(x=as.Date(dates2), y=data$detected.roll), col = cols[5])+
  xlab("")+
...
gNL2b=ggplot() +
  geom_line(data=data, aes(x=as.Date(dates2), y=100*data$positivity.roll.hub), col = cols[5], lty=2)+
  geom_point(data=data, aes(x=as.Date(dates2), y=100*cases.raw.hub/test.raw.hub), col = cols[5], pch=2)+
  geom_point(data=data, aes(x=as.Date(dates2), y=100*cases.raw/test.raw), col = cols[5])+
  geom_line(data=data, aes(x=as.Date(dates2), y=100*data$positivity.roll), col = cols[5])+
  xlab("")+
  ylab("Positivity (%)")+
  ggtitle("Newfoundland and Labrador")+
  theme_classic()+
...
gNL3b=ggplot() +
  geom_line(data=tail(data,14), aes(x=as.Date(dates2), y=test.raw.hub), col = cols[5], lty=2)+
  geom_point(data=tail(data,14), aes(x=as.Date(dates2), y=test.raw.hub), col = cols[5], pch=2)+
...
**Figure 3. Reported new cases (left) and percent positivity (right) used for this analysis: [Department of Health and Community Services Public Advisories](https://www.gov.nl.ca/releases/covid-19-news/) (dots: raw data and solid line: 14-day average) and [Newfoundland and Labrador COVID-19 Pandemic Update Hub](https://geohub-gnl.hub.arcgis.com/search?q=COVID) (open triangles: raw data and dashed line: 14-day average).** The update of this report on March 2 contained an error as "new cases" in the [Newfoundland and Labrador COVID-19 Pandemic Update Hub](https://geohub-gnl.hub.arcgis.com/search?q=COVID) was understood to be since last reporting; if understood as "on the reporting day" then both data sources agree recently. There is some disagreement between the data sources in January, when hundreds of test results completed out-of-province were reported. In the [Department of Health and Community Services Public Advisories](https://www.gov.nl.ca/releases/covid-19-news/) data (manually curated), these cases were assigned to the day they were reported.  

```{r,echo=F}
gNL3+gNL3b
```

**Figure 4. Reported new cases (left) and tests (right) used for this analysis (last 14 days only).** Both the [Department of Health and Community Services Public Advisories](https://www.gov.nl.ca/releases/covid-19-news/) (solid line and dots) and [Newfoundland and Labrador COVID-19 Pandemic Update Hub](https://geohub-gnl.hub.arcgis.com/search?q=COVID) (dashed line and open triangles)  data are shown in both panels, but the data are similar.