...
# Unfortunately quite a lot of this script is not automated and requires manual consideration.
...
# 71 on Jan 14
EH.corr[(i1+4):(ind+4)] = EH.corr[(i1+4):(ind+4)]+EH.corr[(i1+4):(ind+3)]/sum(EH.corr[(i1+4):(ind+4)])*71
...
## Data limitations
On January 12, Dr. Rahman presented the provincial modelling. Several of the graphs provided considered specimen date, rather than report date, and these data represent a more accurate representation of the COVID-19 outbreak in NL.

This report considers only report date, as specimen dates are not reported in the public service advisories.
...
(1) $a_1$ or $a_2$: the number of tests arising due to a contact with a COVID-19 case reported 3 days earlier, and
(2) $b$: the number of tests performed per person in NL;
...
Since $a_2 > a_1$ this means that more tests per case have been performed after the change to recommendations for who should get PCR tested. We conclude that the change in recommendations for PCR testing did not affect the PCR testing rate before and after January 3rd.

### Correcting for the PCR testing changes and out-of-province test results

To correct for the change in PCR testing recommendations (beginning January 3), we assume that beginning January 6th the reporting reflects `r a1-a2` fewer tests per case, and that all these test that did not occur would have been positive. We use this correction so that reported cases can be compared before and after January 6. 
...
## Reported cases (with adjusted data)
...
**Figure 1.** The adjustment for the PCR testing change and out-of-province reporting (line) and reported cases (dots). We assume a 3 day lag for test results such that the January 3 testing change first affects the data on January 6. All 680 cases reported from out-of-province testing on January 10 are assigned to Eastern Health from December 29 to January 6 in proportion to the cases from this date range.
...
The doubling time is the days until the number of reported cases doubles (assuming exponential growth). The halving time is the days until the number of reported cases halves (assuming exponential decay). Our doubling and halving time calculations consider the number of reported cases over the last `r T` days. If the exponential growth coefficient is not different from 0, the number of reported cases is stated as 'At peak'.

**Newfoundland and Labrador: At peak** 

**Eastern Health: At peak** 

**Central Health: At peak** 

**Western Health: At peak** 

**Labrador-Grenfell Health: At peak**

Fitting is performed using nonlinear least squares and considers the data corrected for the changes in PCR testing announced on [January 3](https://www.gov.nl.ca/releases/2022/health/0103n02/).
