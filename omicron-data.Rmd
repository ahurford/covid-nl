...
UK.rate.ad = cumsum(data$EH+data$LG+data$CH+data$WH)/791
ON.rate.ad = cumsum(data$EH+data$LG+data$CH+data$WH)/301
...
The UKHSA study reports for Omicron: 649,834 cases, 815 hospital admissions, and 57 deaths up until December 29, 2021. The PHO study reports for Omicron: 6,312 cases, 21 hospitalizations, and 0 deaths from November 22 - December 17, 2021. These studies were selected as the populations of the United Kingdom and Ontario have more similar age-structure to NL than South Africa, where relevant studies have been completed, but are not considered.
...
**Figure 4.** The reported proportion of hospitalizations for Omicron cases in the UKHSA and PHO studies are parameters that determine the lower and upper bounds of the grey shaded regions for projected cumulative number of hospitalizations since December 15th, 2021 in NL. In addition, we assume hospitalizations are lagged 7 days from cases. The NL Public Advisories do not report the number of new admissions or discharges, so these predictions of the cumulative number of hospitalizations cannot be evaluated using the publicly available information. The range of cumulative hospitalization is [`r round(UK.rate.ad[(L-7)],2)`, `r round(ON.rate.ad[(L-7)],2)`] on `r as.Date(days2[(L-7)], format = "%d %b")` and [`r round(UK.rate.ad[L],2)`, `r round(ON.rate.ad[L],2)`] on `r as.Date(days2[L], format= "%d %b")`.
