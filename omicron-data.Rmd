...
author: "Amy Hurford, Francis Anokye, and Zahra Mohammadi"
output:
...
```
```{r, include=F}
UK.rate = cumsum(data$EH+data$LG+data$CH+data$WH)/648
ON.rate = cumsum(data$EH+data$LG+data$CH+data$WH)/301
...
geom_ribbon(data=data2, aes(x=as.Date(dates), ymin=UK, ymax=ON), fill = "grey70")+
  ...
  ggtitle("Projected cumulative hospitalizations since Dec 15, 2021")+
theme_classic()+
  theme(axis.text.x = element_text(angle = 90),legend.title = element_blank())
g2
```
